...
import { getUserInfo, setUserInfo as fetchSetUserInfo } from '@/api/user';
import { User, UserBasicInfo } from '@/typings/user';
...
const Setting = () => {
  ...
  const handleSubmit = (data: UserInfoForm) => {
    ...
    fetchSetUserInfo(info).then(res => {
        ...
        userInfo ? (
          <>
            ...
            <div className={styles['setting-main']}>
              <h2>账号基本信息</h2>
              <div>
                <Form labelCol={{ offset: 0, span: 3 }} initialValues={userInfo}>
            ...
            <div className={styles['setting-main']}>
              <h2>用户信息</h2>
              <div>
                <Form labelCol={{ offset: 0, span: 3 }} onSubmit={handleSubmit} initialValues={userInfo}>
                  <FormItem label="头像" field="userAvatar" required>
                    <Upload
                      ...
                      fileList={userInfo.avatar ? [{ name: 'avatar', url: `${userInfo.avatar}`, uid: 'avatar' }] : []}
                      headers={{
